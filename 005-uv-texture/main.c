...
SDL_Surface*
APP_LoadImage(Context *context, const char *image_filenamen, int desired_channels)
{
    char full_path[256];
    SDL_Surface *result;
    SDL_PixelFormat format;

    SDL_snprintf(full_path, sizeof(full_path), "%simages/%s", context->base_path, image_filenamen);

    SDL_Log("INFO: Load bmp from paht: %s", full_path);
    ...
    else
    {
        ...
        SDL_Log("ERROR: Unexpected desiredChannels");
        return NULL;
    }
    ...
    SDL_Log("INFO: Image width: %i height: %i", result->w, result->w);
    return result;
}
...
)
{
    ...
    SDL_GPUShaderFormat backend_formats = SDL_GetGPUShaderFormats(context->device);
    ...
    if(backend_formats & SDL_GPU_SHADERFORMAT_SPIRV)
    {
        ...
        SDL_snprintf(
                full_path, 
                sizeof(full_path), "%sshaders/compiled/SPIRV/%s.spv", 
                context->base_path, 
                shader_filename
        );
    ...
    else if(backend_formats & SDL_GPU_SHADERFORMAT_MSL)
    {
      ...
      SDL_snprintf(
              full_path,
              sizeof(full_path), "%sshaders/compiled/MSL/%s.msl", 
              context->base_path, 
              shader_filename);
    ...
    else if(backend_formats & SDL_GPU_SHADERFORMAT_DXIL)
    {
      ...
      SDL_snprintf(
              full_path, 
              sizeof(full_path), "%sshaders/compiled/DXIL/%s.dxil", 
              context->base_path, 
              shader_filename
      );
...
{
    SDL_GPUShader *vertex_shader = APP_LoadShader(context, "default.vert", 0, 0, 0, 0);
    ...
    SDL_GPUShader *fragment_shader = APP_LoadShader(context, "default.frag", 1, 0, 0, 0);
    ...
    SDL_Surface *image_data = APP_LoadImage(context, "default.bmp", 4);
    if(image_data == NULL)
    {
    ...
    SDL_SetGPUBufferName(context->device, context->vertex_buffer, "Vertex Buffer");
    ...
    PositionTextureVertex *transfer_data = SDL_MapGPUTransferBuffer(context->device, buffer_transfer_buffer, false);
    ...
    Uint8 *texture_transfer_ptr = SDL_MapGPUTransferBuffer(context->device, texture_transfer_buffer, false);
    ...
    SDL_GPUCommandBuffer *upload_cmd_buffer = SDL_AcquireGPUCommandBuffer(context->device);
...
{
    SDL_GPUCommandBuffer *cmd_buf = SDL_AcquireGPUCommandBuffer(context->device);
    if(cmd_buf == NULL)
    {
        SDL_Log("ERROR: Failed to acquire gpu command buffer. %s", SDL_GetError());
        return -1;
    }
    ...
    {
        SDL_Log("ERROR: Failed to acquire gpu swapchain texture. %s", SDL_GetError());
        return -1;
    }
...
SDL_AppInit(void **appstate, int argc, char **argv)
{
    ...
    context->device = SDL_CreateGPUDevice(
          SDL_GPU_SHADERFORMAT_SPIRV | SDL_GPU_SHADERFORMAT_MSL | SDL_GPU_SHADERFORMAT_DXIL, false, NULL
    );
    ...
    context->window = SDL_CreateWindow("UV Quad", WINDOW_WIDTH, WINDOW_HEIGHT, 0);
