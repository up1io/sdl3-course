...
SDL_AppInit(void **appstate, int argc, char **argv)
{
    ...
    context->window = SDL_CreateWindow("Vertext Bufffer", WINDOW_WIDTH, WINDOW_HEIGHT, 0);
...
SDL_AppIterate(void *appstate)
{
    ...
    if (!SDL_WaitAndAcquireGPUSwapchainTexture(cmd_buf, context->window, &swapchain_tex, NULL, NULL))
    {
...
{
    SDL_GPUShader *vertex_shader = load_shader(context, "default.vert", 0, 0, 0, 0);
    ...
    SDL_GPUShader *fragment_shader = load_shader(context, "default.frag", 0, 0, 0, 0);
    ...
    context->fill_pipeline = SDL_CreateGPUGraphicsPipeline(context->device, &pipeline_create_info);
    ...
    PositionColorVertex *transfer_data = SDL_MapGPUTransferBuffer(context->device, transfer_buffer, false);
