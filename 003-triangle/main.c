...
static SDL_GPUGraphicsPipeline *fill_pipeline;
static SDL_GPUDevice *device = NULL;

static const char *base_path = NULL;

SDL_GPUShader *load_shader (SDL_GPUDevice *device, const char *shader_filename,
...
SDL_AppInit (void **appstate, int argc, char **argv)
{
  ...
  SDL_GPUShader *vertex_shader
      = load_shader (device, "default.vert", 0, 0, 0, 0);
  ...
  SDL_GPUShader *fragment_shader
      = load_shader (device, "default.frag", 0, 0, 0, 0);
...
SDL_AppIterate (void *appstate)
{
  ...
  if (!SDL_WaitAndAcquireGPUSwapchainTexture (cmd_buf, window, &swapchain_tex,
                                              NULL, NULL))
    {
  ...
  if (swapchain_tex != NULL)
    {
      ...
      // Todo(john): Make it changable between fill and line pipeline
...
SDL_GPUShader *
load_shader (SDL_GPUDevice *device, const char *shader_filename,
             Uint32 sampler_count, Uint32 uniform_buffer_count,
  ...
  if (backend_formats & SDL_GPU_SHADERFORMAT_SPIRV)
    {
      ...
      SDL_snprintf (full_path, sizeof (full_path),
                    "%sshaders/compiled/SPIRV/%s.spv", base_path,
                    shader_filename);
  ...
  else if (backend_formats & SDL_GPU_SHADERFORMAT_MSL)
    {
      ...
      SDL_snprintf (full_path, sizeof (full_path),
                    "%sshaders/compiled/MSL/%s.msl", base_path,
                    shader_filename);
  ...
  else if (backend_formats & SDL_GPU_SHADERFORMAT_DXIL)
    {
      ...
      SDL_snprintf (full_path, sizeof (full_path),
                    "%sshaders/compiled/DXIL/%s.dxil", base_path,
                    shader_filename);
